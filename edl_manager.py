...
class Edl():
    ...
    def __init__(self,title_sufix="") -> None:
        self._date_string = datetime.strftime(datetime.now(),"%Y%m%d_%H%M%S")
        self._edl = {
            "title":self._date_string + title_sufix,
            "cuts":{}
        }
    ...
    def add_cut_in(self,reel:str,clipname:str,tc_in:str,timeline_in:str):
        self._cut_counter+=1
        cut_id = "{:03}".format(self._cut_counter)
        cut = {
            "id":cut_id,
            "reel":reel.zfill(8),
            "clipname":clipname,
    ...
    def save_avid_edl(self,dir:str,filename:str=None):
